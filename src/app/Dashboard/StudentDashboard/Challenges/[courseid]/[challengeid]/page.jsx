...
import { QuizSkeleton } from "@/components/Loader/loading";
...
export default function ChallengeDetailPage() {
  ...
  const challenge = course?.challenges.find(
    (ch) => String(ch._id) === String(challengeid)
  );
  ...
  //         The challenge "{challengeid}" for subject "{courseid}" could not be
  //         found or has no questions.
  //       </p>
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      const res = await fetch(
        `/api/user/${context.user._id}/challenge/${challenge._id}/score`,
        {
          method: "PUT",
      ...
      if (data.success) {
        toast.success(
          `Challenge completed! Score: ${newScore} points. Progress: ${data.progress.toFixed(
            1
          )}%`
        );
            ...
            <CardContent className="pt-4">
             ...
             data-[state=checked]:border-green-400"
                        ...
                        <Label
                          ...
                          className="text-base font-normal cursor-pointer text-gray-800 dark:text-gray-200"
                        >
                          {option.text}
                        </Label>
                      </div>
